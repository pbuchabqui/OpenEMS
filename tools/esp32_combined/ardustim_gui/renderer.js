...
function refreshSerialPorts()
{
    serialport.list().then((ports) => {
        ...
        for(var i = 0; i < ports.length; i++)
        {
            ...
            if(ports[i].vendorId == "10c4" || ports[i].vendorId == "10C4")
            {
              //Silicon Labs CP210x (common ESP32 dev boards)
              newOption.innerHTML = newOption.innerHTML + " (ESP32 CP2102)";
            }
            else if(ports[i].vendorId == "1a86" && ports[i].productId == "7523")
            {
              //CH340 USB-Serial (common on cheap ESP32 boards)
              newOption.innerHTML = newOption.innerHTML + " (ESP32 CH340)";
            }
            else if(ports[i].vendorId == "303a" || ports[i].vendorId == "303A")
            {
              //ESP32-S3/ESP32-C3 native USB
              if(ports[i].productId == "1001") { newOption.innerHTML = newOption.innerHTML + " (ESP32-S3 native USB)"; }
              else { newOption.innerHTML = newOption.innerHTML + " (ESP32 native USB)"; }
            }
            else if(ports[i].vendorId == "2341")
            {
              //Arduino (legacy support)
              if(ports[i].productId == "0010" || ports[i].productId == "0042") { newOption.innerHTML = newOption.innerHTML + " (Arduino Mega)"; }
            }
            else if(ports[i].vendorId == "16c0")
            {
              //Teensy (legacy support)
